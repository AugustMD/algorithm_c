...
void savefile(int *array_pointer) {
	...
	fp = fopen("./output_rquick_100.txt", "a");
	int i = 0;
...
int randomPartition(int *array_pointer, int low, int high) {
    srand(time(NULL));
    int random = low + rand() % (high - low);
    swap(array_pointer + random, array_pointer + high);
...
void quickSort(int *array_pointer, int low, int high) {
	count++;
    if(low < high) {
        int pivot = randomPartition(array_pointer, low, high);
